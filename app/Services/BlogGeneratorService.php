...
class BlogGeneratorService
{
    ...
    public function generateBlogForCategory(Category $category, ?callable $onProgress = null)
    {
        ...
        // 5b. Extract Keywords (Simple frequency + title words fallback)
        // Meta optimization uses topic and title.
        
        // 5c. Link Management & Validation
        // Validate external links generated by AI
        $finalContent = $this->validateAndCleanLinks($finalContent);
    ...
    public function processSeoLinks(string $content, Category $category): string
    {
        // 1. Validate & Clean External Links
        $content = $this->validateAndCleanLinks($content);
        ...
        if ($limitInternal > 0) {
            ...
            if ($relatedBlogs->count() > 0) {
                 $content = $this->insertInternalLinks($content, $relatedBlogs);
            }
        }
    ...
    protected function validateAndCleanLinks(string $html): string
    {
        $dom = new \DOMDocument();
        libxml_use_internal_errors(true);
